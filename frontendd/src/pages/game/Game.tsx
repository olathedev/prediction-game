...
const Game = () => {
  ...
  const { submitPredictions } = useGuessGame(); // Get submitPredictions
  ...
  const handleSelect = (option: string) => {
    ...
    setPredictions((prev) => [...prev, option === currentQuestion?.option1 ? 0 : 1]);
  };
  ...
  useEffect(() => {
    if (gameOver) {
      submitPredictions(predictions, 0.01); // Submitting predictions with a stake amount
      setShowModal(true);
    }
  }, [gameOver]);
          ...
          >
            <motion.button
              ...
              className={`w-[80%] rounded-[2rem] p-4 text-xl font-semibold  text-white transition-all ${
                selectedOption === currentQuestion?.option1 ? "bg-green-500/60" : "bg-[#000]/30"
              }`}
            ...
            <motion.button
              ...
              whileHover={{ scale: 1.1 }}
              className={`w-[80%] rounded-[2rem] p-4 text-xl font-semibold  text-white transition-all ${
                selectedOption === currentQuestion?.option2 ? "bg-green-500/60" : "bg-[#000]/30"
              }`}
