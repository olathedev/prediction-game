...
describe("GuesGame", function () {
  ...
  describe("Set Username", function () {
    ...
    it("Should not allow setting an empty username", async function () {
      await expect(guessGame.connect(addr1).setUsername("")).to.be.revertedWith("Username cannot be empty");
    });

    it("Should not allow setting a duplicate username", async function () {
      await guessGame.connect(addr1).setUsername("Player1");
      await expect(guessGame.connect(addr2).setUsername("Player1")).to.be.revertedWith("Username already taken");
    });
  });
  ...
  describe("Submit Predictions", function () {
    it("Should allow a user to submit predictions and stake ETH", async function () {
      ...
      await guessGame.connect(addr1).submitPredictions(answers, { value: ethers.parseEther("1") });
      const player = await guessGame.getPlayerDetails(addr1.address);
    ...
    it("Should not allow a user to submit predictions without staking ETH", async function () {
      const answers = Array(10).fill(0);
      await expect(guessGame.connect(addr1).submitPredictions(answers)).to.be.revertedWith("Must stake some ETH");
    });

    it("Should not allow a user to submit predictions more than once", async function () {
      const answers = Array(10).fill(0);
      await guessGame.connect(addr1).submitPredictions(answers, { value: ethers.parseEther("1") });
      await expect(guessGame.connect(addr1).submitPredictions(answers, { value: ethers.parseEther("1") })).to.be.revertedWith("Already played");
    });
  });
  ...
  describe("Withdraw Stake", function () {
    ...
    //   const tx = await guessGame.connect(addr1).withdrawStake();
    //   const receipt = await tx.wait();
    //   const gasUsed = receipt?.gasUsed.mul(receipt.effectiveGasPrice);
    //   const finalBalance = await ethers.provider.getBalance(addr1.address);
    //   expect(finalBalance).to.equal(initialBalance.add(ethers.parseEther("1")).sub(gasUsed));
    // });
  });
  ...
  describe("Finish Game", function () {
    ...
    it("Should not allow non-owners to finish the game", async function () {
      await expect(guessGame.connect(addr1).finishGame()).to.be.revertedWith("Only owner can call this");
    });
  });
});
